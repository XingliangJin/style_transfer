...
#here label=style, add conten label
        motions, labels, metas = dataset["motion"], dataset["style"], dataset["meta"]
        content_labels = dataset["content"]

        self.label_i = labels
        #print(content_labels)
#change here
        self.content_label_i = content_labels
...
#here
        content_label = self.content_label_i[index]
...
#add content label(six type walk run jump kick punch trans) and style label
    #获取xia_dataset上的label信息
    ...
    #解析输入的bvh文件名，得到对应的文本label
    filenameforBVH = filename.split('/')[-1]
    style_idx, content_idx, xx = filenameforBVH.split('_')
    #将内容label解析为tensor类型的对应的数
    content_name = content_namedict[int(content_idx) - 1]
    content_idx = content_name_to_idx[content_name]
    content_idx = torch.tensor(int(content_idx))
    content_idx = content_idx[None]
    #将风格label解析为tensor类型的对应的数
    style_idx = style_name_to_idx[style_idx]
