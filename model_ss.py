...
from networks import MLP_label, PatchDis, JointGen, myMLP
...
# co_data, cl_data在data_loader.py里面
    def forward(self, co_data, cl_data, mode):
        ...
        if mode == 'gen_update':
            ...
            # co_data refer to content motion, cl_data refer to style motion 
            la = co_data["label"]
            lb = cl_data["label"]
            ...
            # add style label(total 8) to style code
            print (lb)
            la_one_hot = torch.nn.functional.one_hot(lb, num_classes=8)#指定得到的one_hot编码的长度
            c_la = self.style_mlp(torch.tensor(la_one_hot).float().to(self.device))#用固定长度数据输入到mlp
            c_la = c_la.unsqueeze(-1)
            attention_xa = c_la.mul(s_xa)
            attention_xb = c_la.mul(s_xb)
            attention_xpos = c_la.mul(s_xpos)
            s_xa = torch.mul(torch.sigmoid(attention_xa),s_xa)
            s_xb = torch.mul(torch.sigmoid(attention_xb),s_xb)
            s_xpos = torch.mul(torch.sigmoid(attention_xpos),s_xpos)
